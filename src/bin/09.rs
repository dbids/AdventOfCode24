...
pub fn part_one(input: &str) -> Option<usize> {
  ...
  'Compress: loop {
    ...
    match curr_space.len.cmp(&curr_file.len) {
      Ordering::Greater => {
        //dbg!("Greater", &curr_space, &curr_file);
        files.push_front(Metadata{start:curr_space.start, ..curr_file});
        curr_space.start += curr_file.len;
        curr_space.len -= curr_file.len;
        curr_file = files.pop_back().unwrap_or_else(|| panic!("Ran out of files"));
      }
      Ordering::Equal => {
        //dbg!("Equal", &curr_space, &curr_file);
        files.push_front(Metadata{start:curr_space.start, ..curr_file});
        curr_file = files.pop_back().unwrap_or_else(|| panic!("Ran out of files"));
      ...
      Ordering::Less => {
        //dbg!("Less", &curr_space, &curr_file);
        files.push_front(Metadata{start:curr_space.start, id_num:curr_file.id_num, len:curr_space.len});
