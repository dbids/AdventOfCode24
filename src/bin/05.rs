...
pub fn part_one(input: &str) -> Option<u64> {
  ...
  // Check updates for validity and sum valid ones
...
pub fn part_two(input: &str) -> Option<u64> {
  ...
  // Check updates for validity and sum invalid ones after reordering
  let mut mid_sum: u64 = 0;
  for update in updates_str.split_whitespace() {
    ...
    // iterate over vector, checking rules at each index
    let mut incorrect_at_first = false;
    'fixed: loop {
      ...
      for uv_idx in 0..update_vec.len() {
        ...
        for rule in rules[r_idx].clone().into_iter() {
          if let Some(m_idx) = &update_vec[0..uv_idx].iter().position(|&d| d == rule) {
            ...
            valid_update = false;
            let rule_breaker = update_vec.remove(*m_idx);
            update_vec.insert(uv_idx, rule_breaker);
          }
        }
      }
      // Break out for already correct rules
      if valid_update {
        break 'fixed;
      }
    }
